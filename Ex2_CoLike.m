...
%%        % Method 10: CoLike w/ fminunc w/ trust-region (Analytic gradient)
        % noisepower = reportModel.results.final_iteration.noisepower;
        % noisepower = 1e-1;
        ...
        DOAsbl10 = fminunc(@(theta)mCoLike(theta,Y,noisepower),theta0,optionsOPTunc);

        [ThS,ThI] = sort(DOAsbl10,'descend');
        ThI       = ThI(abs(diff(ThS))<1e-2);
        if ~isempty(ThI)
            for iTr = 1:10
                % iTr
                theta1 = DOAsbl10;
                theta1(ThI) = theta0(ThI);
                DOAsbl10 = fminunc(@(theta)mCoLike(theta,Y,noisepower),theta1,optionsOPTunc);
                [ThS,ThI] = sort(DOAsbl10,'descend');
                ThI       = ThI(abs(diff(ThS))<1e-2);
                if isempty(ThI), break; end
            end
        end

        DoA_error_sbl10 = errorDOAcutoff(DOAsbl10,DOA_src,errCut);
        disp(['RMSE CoLike  grad-A   : ',num2str(sqrt(mean(power(DoA_error_sbl10,2))))])

        [f,g] = mCoLike(DOAsbl10,Y,noisepower);
        disp(['obj.     :                           ',num2str(f)])
        disp(['gradient :                           ',num2str(g.')])

        [f,g] = mCoLike(DOA_src.',Y,noisepower);
        disp(['at True'])
        disp(['obj.     :                           ',num2str(f)])
        disp(['gradient :                           ',num2str(g.')])
...
%%      save
    ...
    saveChar = ['save([ ''zICASSP_CoLike_'', model(1), ''mode_'', ''s'', num2str(Number_of_DOAs), ''MC'' , num2str(NmonteCarlo) , ''SNRn'' , num2str(isnr), ''g'', num2str(M)], ''SNRs'' , ''NmonteCarlo'' '];
    ...
    if isnr > 1
        delete( [ 'zICASSP_CoLike_', model(1), 'mode_', 's', num2str(Number_of_DOAs), 'MC' , num2str(NmonteCarlo) , 'SNRn' , num2str(isnr-1), 'g', num2str(M), '.mat' ] )
    end

end % end of for isnr=1:length(sigma_vec) loop

if mlist > 1
    delete( [ 'zICASSP_CoLike_', model(1), 'mode_', 's', num2str(Number_of_DOAs), 'MC' , num2str(NmonteCarlo) , 'SNRn' , num2str(isnr), 'g', num2str(Mlist(mlist-1)), '.mat' ] )
end
...
for n_output = [1 2]
    ...
    for mInd=1:MInd
        for ind=1:Nsnrs
            ...
            elseif  n_output == 2
                ...
                for index=1:NmonteCarlo
                    totET1 = [totET1;dataLoad((ind-1)*NmonteCarlo+index+ (mInd-1)*NmonteCarlo*Nsnrs).error1];
        ...
        elseif  n_output == 2 && mInd == 1
            hold on; plot(SNRs,rmseSNR1,'*-','Color',pcolor(4,:),'linewidth',1.5,'MarkerSize',8);
            % hold on; plot(SNRs,rmseSNR2,'x','Color',pcolor(3,:),'linewidth',1.5,'LineStyle','--','MarkerSize',8);
            legend('CRB','CoLike','Interpreter','latex','FontSize',14);
